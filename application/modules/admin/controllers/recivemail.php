<?php
class Recivemail extends CI_Controller
{
	var $data = array();
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->recivemail_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->recivemail_model->getlist($key,$offset,$config['per_page']);
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->recivemail_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);
				if (!empty($id)) $this->recivemail_model->delete($id);
			}
