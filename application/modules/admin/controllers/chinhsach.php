...
class Chinhsach extends CI_Controller
{
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->chinhsach_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->chinhsach_model->getlist($key,$offset,$config['per_page']);
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->chinhsach_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);
				if (!empty($id)) $this->chinhsach_model->delete($id);
			}
	...
	function generateSlug($title){
		...
		$suffix = 0;
		while($this->chinhsach_model->getbyslug($slug)){
			if ($suffix)
		...
		try {
			...
			if (!$input){
				...
				if($this->form_validation->run()== FALSE){
					...
					$insert_id = $this->chinhsach_model->add($this->data['value']);
					// -------- Image Upload -----------
					if($_FILES['post-image']['name']){
						...
						if(!empty($this->data['error'])){
							...
							$this->chinhsach_model->update($insert_id, array('image' => $upload_data['file_name']));
						}
					}
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['action'] = strtolower(__FUNCTION__);
				$this->data['post'] = $this->chinhsach_model->getbyid($id);
				...
				if (empty($input)){
					...
					if($this->form_validation->run()== FALSE){
						...
						$this->chinhsach_model->update($id,$this->data['value']);
						$insert_id = $id;
						// -------- Image Upload -----------
						if($_FILES['post-image']['name']){
							$config['upload_path'] = './upload/post';
	...
	function livestatus(){
		try {
			...
			$product = $this->chinhsach_model->getbyid($id);
			if(!empty($product)){
				if($product->status == 1 || $product->status == -1) $this->chinhsach_model->update($id,array('status' => -$product->status));
