...
class Clientuser extends CI_Controller{
	...
	function index(){
		try {
			...
			$config['total_rows'] = $this->clientuser_model->count($key);
			$this->pagination->initialize($config);      //phân trang 
			// load data with parameters 
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->clientuser_model->getlist($key,$offset,$config['per_page']);
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->clientuser_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);  //ko check mà gõ xóa id trên thanh trình duyệt, 
				if (!empty($id)) $this->clientuser_model->delete($id);
			}
			...
			if (!$input){
				...
				if($this->form_validation->run()== FALSE){
					...
					if($_FILES['avatar']['name']){
						$config['upload_path'] = './upload/clientuser';
					...
					if(!empty($this->data['error'])){
						...
						$this->clientuser_model->add($this->data['value']);
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['usergroups'] = $this->usergroups_model->get();
				$this->data['user_info'] = $this->clientuser_model->getbyid($id);
				$this->data['action'] = strtolower(__FUNCTION__);
				$input = $this->input->post();
				//----------------------
				if (empty($input)){
					...
					if($this->form_validation->run()== FALSE){
						...
						if(!empty($this->data['error'])){
							...
							$this->clientuser_model->update($id,$this->data['value']);
	...
	function validusername_check($str){
		$recipient = $this->clientuser_model->getbyusername($str);
		if (!empty($recipient))
		{
	...
	function validemail_check($str){
		$recipient = $this->clientuser_model->getbyemail($str);
		if (!empty($recipient))
		{
