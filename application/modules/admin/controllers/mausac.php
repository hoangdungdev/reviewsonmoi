<?php
class Mausac extends CI_Controller
{
	var $data = array();
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->mausac_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->mausac_model->getlist($key,$offset,$config['per_page']);
	...
	function generateSlug($title){
		...
		$suffix = 0;
		while($this->mausac_model->getbyslug($slug)){
			if ($suffix)
		...
		try {
			...
			if (!$input){
				...
				if($this->form_validation->run()== FALSE){
					...
					$insert_id = $this->mausac_model->add($this->data['value']);					
					// ------------------- images -------------------------
						if($_FILES['post-image']['name']){
						$config['upload_path'] = './upload/mausac/';
						...
						if(!empty($this->data['error'])){
							...
							$this->mausac_model->update($insert_id, array('image' => $upload_data['file_name']));
						}
					}
					// ------------------- images -------------------------
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['action'] = strtolower(__FUNCTION__);
				$this->data['post'] = $this->mausac_model->getbyid($id);
				$input = $this->input->post();
				//----------------------
				if (empty($input)){
					$this->layout->view(strtolower(__CLASS__).'/modify',$this->data);
					...
					if($this->form_validation->run()== FALSE){
						...
						$this->mausac_model->update($id,$this->data['value']);
						$insert_id = $id;
						// ------------------- images -------------------------
						if($_FILES['post-image']['name']){
						$config['upload_path'] = './upload/mausac/';
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->mausac_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);
				if (!empty($id)) $this->mausac_model->delete($id);
			}
	...
	function livestatus(){
		try {
			...
			$product = $this->mausac_model->getbyid($id);
			if(!empty($product)){
				if($product->status == 1 || $product->status == -1) $this->mausac_model->update($id,array('status' => -$product->status));
