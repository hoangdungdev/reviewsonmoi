...
class Banner extends CI_Controller
{
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->banner_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->banner_model->getlist($key,$offset,$config['per_page']);
	...
	function generateSlug($title){
		...
		$suffix = 0;
		while($this->banner_model->getbyslug($slug)){
			if ($suffix)
	...
	function add(){
		...
		// 										'status' => $input['status']
		// 			);
		// 			$insert_id = $this->banner_model->add($this->data['value']);					
		// 			// ------------------- images -------------------------
		...
		// 					$this->layout->view(strtolower(__CLASS__).'/modify',$this->data);
		// 				} else {
		// 					$this->banner_model->update($insert_id, array('image' => $upload_data['file_name']));
		// 				}
		// 			}
		// 			// ------------------- images -------------------------
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['action'] = strtolower(__FUNCTION__);
				$this->data['post'] = $this->banner_model->getbyid($id);
				$input = $this->input->post();
				//----------------------
				if (empty($input)){
					$this->layout->view(strtolower(__CLASS__).'/modify',$this->data);
					...
					if($this->form_validation->run()== FALSE){
						...
						$this->banner_model->update($id,$this->data['value']);
						$insert_id = $id;
						// ------------------- images -------------------------
						if($_FILES['post-image']['name']){
						$config['upload_path'] = './upload/banner/';
	...
	function delete(){
		...
		// 	if(!empty($id)){
		// 		foreach ($id as $row):
		// 		$this->banner_model->delete($row);
		// 		endforeach;
		// 	}
		// 	else{
		// 		$id = $this->uri->segment(4);
		// 		if (!empty($id)) $this->banner_model->delete($id);
		// 	}
	...
	function livestatus(){
		try {
			...
			$product = $this->banner_model->getbyid($id);
			if(!empty($product)){
				if($product->status == 1 || $product->status == -1) $this->banner_model->update($id,array('status' => -$product->status));
