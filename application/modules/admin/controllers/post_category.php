...
class Post_category extends CI_Controller
{
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->post_category_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->post_category_model->getmenu();
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->post_category_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);
				if (!empty($id)) $this->post_category_model->delete($id);
			}
		...
		try {
			...
			if (!$input){
				...
				if($this->form_validation->run()== FALSE){
					...
					$this->post_category_model->add($this->data['value']);
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['action'] = strtolower(__FUNCTION__);
				$this->data['post_category'] = $this->post_category_model->getbyid($id);
				$this->data['categories'] = $this->post_category_model->getmenu();
				$input = $this->input->post();
				//------------------
				if (!$input){
					...
					if($this->form_validation->run()== FALSE){
						...
						$this->post_category_model->update($id,$this->data['value']);
	...
	function generateSlug($title){
		...
		$suffix = 0;
		while($this->post_category_model->getbyslug($slug)){
			if ($suffix)
