...
class Bank extends CI_Controller
{
	...
	function index()
	{
		try {
			...
			$config['total_rows'] = $this->bank_model->count($key);
			$this->pagination->initialize($config);
			// load data with parameters
			$offset = $this->input->get('per_page');
			$this->data['total_rows'] = $config['total_rows'];
			$this->data['dataset'] = $this->bank_model->getlist($key,$offset,$config['per_page']);
	...
	function generateSlug($title){
		...
		$suffix = 0;
		while($this->bank_model->getbyslug($slug)){
			if ($suffix)
	...
	function add(){
		try {
			...
			if (!$input){
				...
				$this->form_validation->set_rules('name', 'Tên Chủ Tài Khoản', 'min_length[1]|max_length[255]|trim|required');
				$this->form_validation->set_rules('title', 'Số TK', '');
				...
				if($this->form_validation->run()== FALSE){
					...
					$insert_id = $this->bank_model->add($this->data['value']);					
					// ------------------- images -------------------------
						...
						if(!empty($this->data['error'])){
							...
							$this->bank_model->update($insert_id, array('image' => $upload_data['file_name']));
						}
					}
					// ------------------- images -------------------------
	...
	function edit(){
		try {
			...
			if (isset($id)){
				$this->data['action'] = strtolower(__FUNCTION__);
				$this->data['post'] = $this->bank_model->getbyid($id);
				$input = $this->input->post();
				//----------------------
				if (empty($input)){
					$this->layout->view(strtolower(__CLASS__).'/modify',$this->data);
					...
					if($this->form_validation->run()== FALSE){
						...
						$this->bank_model->update($id,$this->data['value']);
						$insert_id = $id;
						// ------------------- images -------------------------
						if($_FILES['post-image']['name']){
						$config['upload_path'] = './upload/bank/';
	...
	function delete(){
		try {
			...
			if(!empty($id)){
				foreach ($id as $row):
				$this->bank_model->delete($row);
				endforeach;
			}
			else{
				$id = $this->uri->segment(4);
				if (!empty($id)) $this->bank_model->delete($id);
			}
	...
	function livestatus(){
		try {
			...
			$product = $this->bank_model->getbyid($id);
			if(!empty($product)){
				if($product->status == 1 || $product->status == -1) $this->bank_model->update($id,array('status' => -$product->status));
